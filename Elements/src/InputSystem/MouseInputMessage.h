...
class MouseMoveMessage : public Message {
 public:
  MouseMoveMessage(std::any data) : Message{data} {
...
class MouseScrollMessage : public Message {
 public:
  MouseScrollMessage(std::any data) : Message{data} {
    type = MessageType::MouseScroll;
  }

  float getXOffset() {
    return std::any_cast<std::pair<float, float>>(data).first;
  }

  float getYOffset() {
    return std::any_cast<std::pair<float, float>>(data).second;
  }
};

class MouseButtonDownMessage : public Message {
 public:
  MouseButtonDownMessage(std::any data) : Message{data} {
...
class MouseButtonUpMessage : public Message {
 public:
  MouseButtonUpMessage(std::any data) : Message{data} {
